...
```{r}
experiment.path = "/gpfs/scratch/citrod01/experiments/Zimbabwe/ZWE_Phase2/Baseline-campaign_ZWE_MIHPSA_phase2_tdf_plw-tdf_plw___2024_10_22_12_56_31_477990/"
...
HIVDeaths.summary %>% ggplot() + 
  geom_line(mapping = aes(x = Year, y = AIDSDeaths_F, color = scenario_name ))
```
...
p1 <- sim.results.all %>% 
  ...
  filter(Age >=15, Age < 25, Gender == 1) %>%
  group_by(Year, scenario_name, sim.id) %>% 
  summarize(NTDFPrEP_F1524_M = sum(ReceivePrEP_HIGH * pop.scaling.factor), 
            .groups = 'keep') %>% 
  group_by(Year, scenario_name) %>% 
  summarize(NTDFPrEP_F1524_M = mean(NTDFPrEP_F1524_M), 
            .groups = 'keep') %>% 
  ungroup()
...
### Oral PrEP for SD couples
```{r}
# NTDFPrEP_SDCA1599_M
sd.summary <- sim.results.all %>% 
  mutate(Year = ceiling(Year)) %>% 
  filter(Age >=15) %>%
  group_by(Year, scenario_name, sim.id) %>% 
  summarize(NTDFPrEP_SDCA1599_M = sum(ReceivePrEP_HIGH * pop.scaling.factor), 
            .groups = 'keep') %>% 
  group_by(Year, scenario_name) %>% 
  summarize(NTDFPrEP_SDCA1599_M = mean(NTDFPrEP_SDCA1599_M), 
            .groups = 'keep') %>% 
  ungroup()

sd.summary %>% 
  ggplot() + 
  geom_point(mapping = aes(x = Year, y = NTDFPrEP_SDCA1599_M, color = scenario_name))
```

### Oral PrEP for PLW
```{r}
# NTDFPrEP_pregbfF1549_M
plw.summary <- sim.results.all %>% 
  mutate(Year = ceiling(Year)) %>% 
  filter(Age >=15, Age < 50, Gender == 1) %>%
  group_by(Year, scenario_name, sim.id) %>% 
  summarize(NTDFPrEP_pregbfF1549_M = sum(ReceivePrEP_HIGH * pop.scaling.factor), 
            .groups = 'keep') %>% 
  group_by(Year, scenario_name) %>% 
  summarize(NTDFPrEP_pregbfF1549_M = mean(NTDFPrEP_pregbfF1549_M), 
            .groups = 'keep') %>% 
  ungroup()

plw.summary %>% 
  ggplot() + 
  geom_point(mapping = aes(x = Year, y = NTDFPrEP_pregbfF1549_M, color = scenario_name))
```
