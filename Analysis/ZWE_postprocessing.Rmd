...
```{r}
#res.path.noprep = "/gpfs/data/bershteynlab/EMOD/citrod01/ZWE_calib_20220627/ZWE_phase2_2/Baseline-campaign_ZWE_MIHPSA_2023_noprep-noprep/ReportHIVByAgeAndGender"

# Results generated prior to changing the testing rates
res.path.noprep = "/gpfs/scratch/citrod01/experiments/Zimbabwe/Baseline-campaign_ZWE_MIHPSA_2023_noprep-noprep___2023_10_03_13_03_43_434191"

sim.results.essential <- EMODAnalyzeR::read.simulation.results.bigpurple(
  experiment_path = res.path.noprep,
  ...
  summarize_columns = c("Population","Infected", "On_ART",
                        ...
                        "ReceivePrEP_HIGH", "Program_VMMC"),
  stratify_columns = c("Year", "Gender", "Age", "IP_Key.Risk", "IsCircumcised"),
  min_age_inclusive = 0,
  max_age_inclusive = 99
)

#  Results using adjusted testing rates after 2018
#res.path.noprep = "/gpfs/data/bershteynlab/EMOD/citrod01/ZWE_calib_20220627/ZWE_phase1_2_bad_testing/Baseline-campaign_ZWE_MIHPSA_2023_noprep-phase1/ReportHIVByAgeAndGender"

res.path.noprep = "/gpfs/data/bershteynlab/EMOD/citrod01/ZWE_calib_20220627/ZWE_phase1_2/Baseline-campaign_ZWE_MIHPSA_2023_noprep-phase1/ReportHIVByAgeAndGender"

sim.results.essential <- EMODAnalyzeR::read.simulation.results(
  results_path  = res.path.noprep,
  scenario_name = 'noprep',
  summarize_columns = c("Population","Infected", "On_ART",
                        "Died", "Died_from_HIV",
                        "Newly.Infected","Diagnosed",
...
#res.path.prep = "/gpfs/data/bershteynlab/EMOD/citrod01/ZWE_calib_20220627/ZWE_phase2/Baseline-campaign_ZWE_MIHPSA_2023-prep/ReportHIVByAgeAndGender"

res.path.prep = "/gpfs/scratch/citrod01/experiments/Zimbabwe/Baseline-campaign_ZWE_MIHPSA_2023-prep___2023_10_03_13_05_25_712004"

sim.results.prep <- EMODAnalyzeR::read.simulation.results.bigpurple(
  experiment_path = res.path.prep,
  scenario_name = 'prep',
  summarize_columns = c("Population","Infected", "On_ART",
                        "Died", "Died_from_HIV",
                        "Newly.Infected","Diagnosed", 
...
```{r}
nruns = sim.results.essential$sim.ix %>% unique %>% length()
...
Count tests over time in adults:

```{r}
# These are the data in the phase1 paper
test.dat <- data.table(
  year = c(2011, 2012, 2013,2014, 2015, 2016, 2017, 2018, 2019, 2020),
  tests = c(1656719,	1868842,	1430246,	1756524,	2201250,	2664844,	3174630,	3011027,	2382768,	1521942)
)
```
...
p3 %>% ggplot() + 
  geom_line(mapping = aes(x = Year, y = TOTTests_A1599_M)) + 
  geom_point(data = test.dat, mapping = aes(x = year, y = tests))
...
NB: we distribute VMMC to 10-25 year olds, which is slightly different from the metric we are being asked for here.

### PrEP scenario

```{r}
VMMC_prep <- sim.results.prep %>% 
  filter(Age >=10, Age < 50) %>% 
  group_by(Year, sim.ix, scenario_name) %>% 
  summarize(TOT_VMMC_M1549_M = sum(Program_VMMC*pop.scaling.factor), .groups = "keep") %>% 
  group_by(Year) %>% 
  summarize(TOT_VMMC_M1549_M_sd = sd(TOT_VMMC_M1549_M), 
            TOT_VMMC_M1549_M = mean(TOT_VMMC_M1549_M), 
            .groups = 'keep') %>% 
  ungroup() %>% 
  mutate(
    CIRC_PREV_M1549_95LL = lower_ci(TOT_VMMC_M1549_M, TOT_VMMC_M1549_M_sd/sqrt(nruns), nruns, conf_level = .95),
    CIRC_PREV_M1549_95UL = upper_ci(TOT_VMMC_M1549_M, TOT_VMMC_M1549_M_sd/sqrt(nruns), nruns, conf_level = .95)
  ) %>% 
  select(-TOT_VMMC_M1549_M_sd)
```
...
```{r}
VMMC_essential <- sim.results.essential %>% 
  filter(Age >=10, Age < 50) %>% 
  group_by(Year, sim.ix, scenario_name) %>% 
  summarize(TOT_VMMC_M1549_M = sum(Program_VMMC*pop.scaling.factor), .groups = "keep") %>% 
  group_by(Year) %>% 
  ...
  ungroup() %>% 
  mutate(
    TOT_VMMC_M1549_95LL = lower_ci(TOT_VMMC_M1549_M, TOT_VMMC_M1549_M_sd/sqrt(nruns), nruns, conf_level = .95),
    TOT_VMMC_M1549_95UL = upper_ci(TOT_VMMC_M1549_M, TOT_VMMC_M1549_M_sd/sqrt(nruns), nruns, conf_level = .95)
  ) %>% 
  select(-TOT_VMMC_M1549_M_sd)
```
...
ggplot() + 
  # geom_point(
  #   data = VMMC_prep,
  #   mapping = aes(x = Year, y = TOT_VMMC_M1549_M), color = 'red') + 
  # geom_errorbar(
  #   data = VMMC_prep,
  #   mapping = aes(x = Year, ymin = TOT_VMMC_M1549_95LL, ymax = TOT_VMMC_M1549_95UL), color = 'red') + 
  geom_point(
    data = VMMC_essential,
    mapping = aes(x = Year, y = TOT_VMMC_M1549_M), color = 'blue') +
  geom_errorbar(
    data = VMMC_essential,
    mapping = aes(x = Year, ymin = TOT_VMMC_M1549_95LL, ymax = TOT_VMMC_M1549_95UL), color = 'blue')
```

### Save

```{r}
# write_csv(VMMC_essential, "ZWE_MIHPSA_VMMC_essential.csv")

# write_csv(VMMC_prep, "ZWE_MIHPSA_VMMC_prep.csv")
```

## VMMC - Proportion of men Age 15-49 who are circumcised

```{r}

sim.results.essential %>% colnames

CIRC_PREV_essential <- sim.results.essential %>% 
  filter(Age >=10, Age < 50, Gender == 0) %>% 
  group_by(Year, sim.ix, scenario_name, IsCircumcised) %>% 
  summarize(Population = sum(Population), .groups = 'keep') %>% 
  ungroup() %>%
  pivot_wider(id_cols = c(Year, sim.ix,scenario_name), values_from = c(Population), 
              names_from = c(IsCircumcised), names_prefix = "IsCircumcised") %>% 
  mutate(tot = IsCircumcised0 + IsCircumcised1) %>% 
  mutate(CIRC_PREV_M1549_M = case_when(tot == 0 ~ 0,
                                       tot > 0 ~ IsCircumcised1/tot)) %>% 
  group_by(Year) %>% 
  ...
  ungroup() %>% 
  mutate(
    CIRC_PREV_M1549_95LL = lower_ci(CIRC_PREV_M1549_M, CIRC_PREV_M1549_M_sd/sqrt(nruns), nruns, conf_level = .95),
    CIRC_PREV_M1549_95UL = upper_ci(CIRC_PREV_M1549_M, CIRC_PREV_M1549_M_sd/sqrt(nruns), nruns, conf_level = .95)
  ) %>% 
...
CIRC_PREV_essential %>% 
  ggplot() + 
  geom_line(mapping = aes(x = Year, y = CIRC_PREV_M1549_M)) + 
  ylim(0,1)
```
### Save

```{r}
merge(VMMC_essential, CIRC_PREV_essential, 
      by = "Year") %>% 
  write_csv(file = "ZWE_MIHPSA_CIRC_essential.csv")
```
...
ylds.current.prep %>% filter(Year >= 2023) %>% 
  ...
  merge(HIVprev.prep %>% filter(Year >= 2023),
        by = "Year") %>% 
  merge(VMMC_prep %>% filter(Year >= 2023),
        by = "Year") %>% 
...
ylds.current.base %>% filter(Year >= 2023) %>% 
  ...
  merge(HIVprev.base %>% filter(Year >= 2023),
        by = "Year") %>% 
    merge(VMMC_essential %>% filter(Year >= 2023),
        by = "Year") %>% 
